...
```bash
export API_AUTH_INSECURE_SHARED_KEY=your-api-key
```
...
The `API_AUTH_INSECURE_SHARED_KEY` can be stored in AWS SSM Parameter Store and retrieved via:
...
If using template-infra, add this to your [app env-config](https://github.com/navapbc/template-infra/blob/main/docs/infra/environment-variables-and-secrets.md#secrets):
