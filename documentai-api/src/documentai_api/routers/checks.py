"""Check endpoint - returns per-check pass/fail/not_evaluated breakdown for a document."""
...
from documentai_api.models.checks import CheckEntry, CheckResponse
from documentai_api.schemas.document_metadata import DocumentMetadata
from documentai_api.utils.auth import get_user_context_from_api_key
from documentai_api.utils.checks import (
    CHECKLIST,
    CheckKey,
    CheckStatus,
    NotEvaluatedReason,
...
_NOT_EVALUATED = CheckStatus.NOT_EVALUATED
...
_STOP_FAIL_REASONS: dict[str, str] = {
    CheckKey.PASSWORD_PROTECTED: "Document is password protected.",
    CheckKey.DOCUMENT_DETECTED: "Insufficient text detected to identify a document.",
    CheckKey.MULTIPLE_DOCUMENTS_ON_SINGLE_PAGE: "Multiple documents were detected on a single page.",
    CheckKey.MULTIPLE_DOCUMENTS_IN_MULTIPAGE: "Pages are not continuations of a single document instance.",
}

_BLUR_STOP_FALLBACK = "Document was flagged as blurry."


def _stop_fail_entry(key: str, ddb_record: dict[str, Any]) -> CheckEntry:
    """Return a guaranteed-fail entry for the stop key, driven by the response code."""
    if key == CheckKey.BLUR:
        reason = ddb_record.get(DocumentMetadata.IS_DOCUMENT_BLURRY_REASON) or _BLUR_STOP_FALLBACK
        return CheckEntry(status=_FAIL, reason=reason)
    return CheckEntry(status=_FAIL, reason=_STOP_FAIL_REASONS[key])


def _evaluate_key(key: str, ddb_record: dict[str, Any]) -> CheckEntry:
    """Return pass/fail for a single pipeline key using its own stored signal."""
    if key == CheckKey.PASSWORD_PROTECTED:
        if ddb_record.get(DocumentMetadata.IS_PASSWORD_PROTECTED):
            return CheckEntry(status=_FAIL, reason="Document is password protected.")
        return CheckEntry(status=_PASS, reason="Document is not password protected.")

    if key == CheckKey.DOCUMENT_DETECTED:
        # No stored boolean - reaching this key means OCR succeeded.
        return CheckEntry(status=_PASS, reason="Sufficient text detected.")
    ...
    if key == CheckKey.BLUR:
        ...
        if is_blurry:
            return CheckEntry(status=_FAIL, reason=reason)
        return CheckEntry(status=_PASS, reason=reason)

    if key == CheckKey.MULTIPLE_DOCUMENTS_ON_SINGLE_PAGE:
        # No stored boolean - reaching this key means only one document was detected.
        return CheckEntry(status=_PASS, reason="No multiple documents detected.")

    if key == CheckKey.MULTIPLE_DOCUMENTS_IN_MULTIPAGE:
    ...
    if key == CheckKey.MISCATEGORIZATION:
        ...
        if category_match is False:
            ...
            return CheckEntry(status=_FAIL, reason=detail)
        return CheckEntry(status=_PASS, reason=None)

    if key == CheckKey.MISSING_FIELDS:
        if DocumentMetadata.EXTRACTION_RULES_CONFIGURED not in ddb_record:
            return CheckEntry(status=_NOT_EVALUATED, reason=NotEvaluatedReason.LEGACY_DOCUMENT)
        if ddb_record.get(DocumentMetadata.EXTRACTION_RULES_CONFIGURED) is False:
            return CheckEntry(status=_NOT_EVALUATED, reason="Extraction rules not configured.")
    ...
    if key == CheckKey.EXTRACTION_CONFIDENCE:
    ...
    return CheckEntry(status=_NOT_EVALUATED, reason=None)


def _build_checks(response_code: str | None, ddb_record: dict[str, Any]) -> dict[str, CheckEntry]:
    """Build the full fixed check key set for a given response code."""
    if response_code is None:
        return {
            key: CheckEntry(status=_NOT_EVALUATED, reason=NotEvaluatedReason.STOPPED_INTERNAL_ERROR)
    ...
    if response_code == ResponseCodes.NO_BLUEPRINT_MATCHED:
        ...
        if ddb_record.get(DocumentMetadata.BDA_INVOCATION_ARN):
            return {key: _evaluate_key(key, ddb_record) for key in CHECKLIST}
        return {
            key: CheckEntry(
    ...
    if response_code in (
        ...
        return {key: CheckEntry(status=_NOT_EVALUATED, reason=reason) for key in CHECKLIST}
    ...
    if stopped_pre_extraction is None:
        ...
        logger.warning(
            f"Unrecognized response code in _build_checks: {response_code!r} - defaulting to not_evaluated"
        )
        return {
            key: CheckEntry(status=_NOT_EVALUATED, reason=NotEvaluatedReason.STOPPED_INTERNAL_ERROR)
            for key in CHECKLIST
    ...
    stop_index = CHECKLIST.index(stop_key)

    result: dict[str, CheckEntry] = {}
    for i, key in enumerate(CHECKLIST):
        if i < stop_index:
            result[key] = _evaluate_key(key, ddb_record)
        elif i == stop_index:
            result[key] = _stop_fail_entry(key, ddb_record)
        else:
            result[key] = CheckEntry(status=_NOT_EVALUATED, reason=not_evaluated_reason)

    return result


@router.get("/v1/documents/{job_id}/check", tags=["Documents:Query"])
async def get_document_checks(job_id: uuid.UUID, auth: AuthUser) -> CheckResponse:
    """Return a per-check breakdown for a processed document."""
    ...
    checks = _build_checks(response_code, job_status.ddb_record)
