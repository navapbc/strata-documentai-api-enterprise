...
from documentai_api.classifiers import api_response as api_response_util
...
def test_build_v1_api_response(
    ...
    response = api_response_util.build_v1_api_response(
...
def test_build_v1_api_response_no_record(ddb_doc_metadata_table):
    with pytest.raises(ValueError, match="DDB record not found for file: test-does-not-exist"):
        api_response_util.build_v1_api_response(
            "test-does-not-exist",
...
def test_build_v1_api_response_empty_record(ddb_doc_metadata_table):
    ddb_doc_metadata_table.put_item(Item={DocumentMetadata.FILE_NAME: "test-key"})
...
def test_build_v1_api_response_with_bounding_box(
    s3_bucket, ddb_doc_metadata_table, bda_result_with_geometry
):
    import json
...
def test_build_v1_api_response_applies_extraction_rules(
    s3_bucket, ddb_doc_metadata_table, extraction_rules_table, mocker
):
    import json
    ...
    bda_obj = s3_bucket.put_object(Key="key.json", Body=json.dumps(bda_results))
    ...
    response = api_response_util.build_v1_api_response("test-key", ProcessStatus.SUCCESS.value)
...
def test_build_v1_api_response_missing_geometry_and_empty_fields_trigger_101(
    s3_bucket, ddb_doc_metadata_table, extraction_rules_table, monkeypatch
):
    import json
    ...
    ddb_doc_metadata_table.put_item(
        Item={
            ...
            DocumentMetadata.CREATED_AT: "2026-01-01T00:00:00+00:00",
            DocumentMetadata.FIELD_CONFIDENCE_SCORES: '[{"GrossPay": 0.95}, {"PayPeriodStartDate": 0.12}, {"YTDGrossPay": 0.94}]',
    ...
    monkeypatch.setattr(
        "documentai_api.utils.ssm.is_missing_geo_included_with_missing_fields", lambda: True
    )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (True, False, ResponseCodes.MISCATEGORIZED),
    ],
)
def test_build_v1_api_response_success_response_code_precedence(
    ...
    if category_match is not None:
        record[DocumentMetadata.PRECLASSIFICATION_CATEGORY_MATCH] = category_match
...
def test_user_category_echoed_when_present(s3_bucket, ddb_doc_metadata_table):
    ...
    response = api_response_util.build_v1_api_response("user-cat", ProcessStatus.SUCCESS.value)
...
def test_user_category_absent_when_not_set(s3_bucket, ddb_doc_metadata_table):
    ...
    response = api_response_util.build_v1_api_response("no-user-cat", ProcessStatus.SUCCESS.value)
...
def test_102_includes_inferred_document_type(s3_bucket, ddb_doc_metadata_table):
    ...
    response = api_response_util.build_v1_api_response("102-suggested", ProcessStatus.SUCCESS.value)
...
def test_inferred_document_type_present_on_non_102(s3_bucket, ddb_doc_metadata_table):
    ...
    response = api_response_util.build_v1_api_response("100-detected", ProcessStatus.SUCCESS.value)

    assert response["responseCode"] == ResponseCodes.SUCCESS
    assert response["inferredDocumentType"] == "pay stub"


def test_inferred_document_type_absent_when_not_preclassified(s3_bucket, ddb_doc_metadata_table):
    ddb_doc_metadata_table.put_item(Item=_minimal_success_record(s3_bucket, "100-no-detect"))

    response = api_response_util.build_v1_api_response("100-no-detect", ProcessStatus.SUCCESS.value)
...
# =============================================================================
# _build_field_map - pure unit tests
# =============================================================================


def test_build_field_map_redacts_when_not_include_extracted():
    result = api_response_util._build_field_map(
...
def test_build_v1_api_response_extraction_rules_match_nested_fields(
    ...
    """present_v1_response nests verbatim dotted names for client presentation."""
    import json
    from datetime import UTC, datetime
    ...
    ddb_doc_metadata_table.put_item(
        Item={
            ...
            DocumentMetadata.BDA_COMPLETED_AT: bda_completed_at.isoformat(),
            DocumentMetadata.CREATED_AT: created_at.isoformat(),
            DocumentMetadata.FIELD_CONFIDENCE_SCORES: '[{"applicant.first_name": 0.95}, {"applicant.last_name": 0.9}, {"extra_field": 0.8}]',
            "tenantId": "t1",
        }
    )
    extraction_rules_table.put_item(
    ...
    presented = present_v1_response(response)
