...
def test_process_batch_empty_queue():
    ...
    with patch(
        "documentai_api.jobs.metrics_processor.main.sqs_service.receive_messages"
    ) as mock_receive:
...
def test_process_batch_success(s3_client, s3_bucket):
    ...
    with (
        ...
        patch(
            "documentai_api.jobs.metrics_processor.main.sqs_service.delete_message"
        ) as mock_delete,
    ):
...
def test_main_processes_multiple_batches(s3_client, s3_bucket):
    ...
    with (
        ...
        patch("documentai_api.jobs.metrics_processor.main.sqs_service.delete_message"),
    ):
